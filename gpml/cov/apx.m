...
elseif grid                                            % C)  Grid approximations
  ...
  if isfield(opt,'cg_showit'), cgshowit = opt.cg_showit; % show number of iterate
  else cgshowit = 0; end
  ...
  cgpar = {cgtol,cgmit,cgshowit}; xg = cov{3}; p = numel(xg);  % conjugate gradient, grid
...
function q = linsolve(p,mvm,varargin) % solve q = mvm(p) via conjugate gradients
  [q,flag,relres,iter] = conjgrad(mvm,p,varargin{1:2});                 % like pcg
  cgshowit = varargin{3};
  if ~flag
      if cgshowit==1
        fprintf('Not converged, r=%1.2e\n',relres);
      end
